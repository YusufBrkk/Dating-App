...
// Veritabanından çalışma saatleri bilgisini çek
$query = "SELECT baslangic, bitis, haftalik_tatil FROM calisma_saatleri LIMIT 1";
...
if ($row = mysqli_fetch_assoc($result)) {
    ...
    $bitis = $row['bitis'];
    $haftalik_tatil = $row['haftalik_tatil'];
}
?>
...
<style>
    ...
    .calisma-saatleri-banner i {
        font-size: 2.7rem;
        margin-bottom: 0.5rem;
        color: #fff;
        filter: drop-shadow(0 2px 8px #2563eb55);
    }
    .calisma-saatleri-banner h2 {
        font-weight: 800;
        font-size: 2rem;
        margin-bottom: 0.2rem;
        letter-spacing: 1px;
    }
    .calisma-saatleri-banner .desc {
        font-size: 1.08rem;
        color: #e0e7ef;
        margin-bottom: 0;
    }
    .calisma-saatleri-list {
        background: transparent;
        padding: 2rem 1.5rem 1.2rem 1.5rem;
    }
    .calisma-saatleri-list .list-group-item {
        background: #f8fafc;
        border: none;
    ...
    .calisma-saatleri-list .list-group-item i {
        color: #2563eb;
        font-size: 1.3rem;
        margin-right: 0.7rem;
    }
    .calisma-saatleri-list .fw-bold {
        color: #2563eb;
        font-size: 1.15rem;
        letter-spacing: 0.5px;
    }
    .calisma-saatleri-info {
    ...
    .calisma-saatleri-footer {
        padding: 0 1.5rem 1.5rem 1.5rem;
    }
    .calisma-saatleri-footer .btn {
        width: 100%;
        font-size: 1.1rem;
        border-radius: 0.7rem;
        font-weight: 600;
...
<?php if (!isset($_SESSION["adminID"]) && isset($_SESSION["userID"])): ?>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <nav class="nav flex-column mt-2">
            <a href="profile-update.php" class="nav-link<?= basename($_SERVER['PHP_SELF']) == 'profile-update.php' ? ' active' : '' ?>">
                <i class="bi bi-person-circle"></i>
                <span class="sidebar-link-text">Müşteri</span>
            </a>
...
<div class="container d-flex flex-column align-items-center justify-content-center" style="min-height: 90vh;">
    <div class="card calisma-saatleri-card rounded-4">
        <div class="calisma-saatleri-banner">
            <i class="bi bi-clock-history"></i>
            <h2>Çalışma Saatlerimiz</h2>
            <div class="desc">Randevu alabileceğiniz saat ve tatil günleri</div>
        </div>
        <div class="calisma-saatleri-list">
            <div class="list-group list-group-flush mb-3">
                <div class="list-group-item d-flex justify-content-between align-items-center">
                    <span><i class="bi bi-clock"></i>Başlangıç Saati</span>
                    <span class="fw-bold">
    <?= htmlspecialchars(date('H.i', strtotime($baslangic))) ?>
</span>
                </div>
                <div class="list-group-item d-flex justify-content-between align-items-center">
                    <span><i class="bi bi-clock"></i>Bitiş Saati</span>
                    <span class="fw-bold">
    <?= htmlspecialchars(date('H.i', strtotime($bitis))) ?>
</span>
                </div>
                <div class="list-group-item d-flex justify-content-between align-items-center">
                    <span><i class="bi bi-calendar-x"></i>Haftalık Tatil</span>
                    <span class="fw-bold"><?= htmlspecialchars($haftalik_tatil) ?></span>
                </div>
            </div>
        </div>
        <div class="calisma-saatleri-info">
            <i class="bi bi-info-circle me-2"></i>
            Lütfen randevu alırken bu saat ve günlere dikkat ediniz.
        </div>
