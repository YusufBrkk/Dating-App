...
$stmt = mysqli_prepare($connection, "SELECT ad_soyad, telefon FROM musteriler WHERE musteriID = ?");
mysqli_stmt_bind_param($stmt, "i", $userID);
mysqli_stmt_execute($stmt);
mysqli_stmt_bind_result($stmt, $ad_soyad, $telefon);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $new_telefon = trim($_POST["telefon"]);
    $new_password = trim($_POST["password"]);

    if ($new_ad_soyad === "" || $new_telefon === "") {
        $error = "Ad Soyad ve Telefon alanları boş bırakılamaz!";
    } else {
        if ($new_password !== "") {
            // Şifre güncellenecek
            $hashed = password_hash($new_password, PASSWORD_DEFAULT);
            $stmt = mysqli_prepare($connection, "UPDATE musteriler SET ad_soyad=?, telefon=?, sifre=? WHERE musteriID=?");
            mysqli_stmt_bind_param($stmt, "sssi", $new_ad_soyad, $new_telefon, $hashed, $userID);
        } else {
            // Şifre güncellenmeyecek
            $stmt = mysqli_prepare($connection, "UPDATE musteriler SET ad_soyad=?, telefon=? WHERE musteriID=?");
            mysqli_stmt_bind_param($stmt, "ssi", $new_ad_soyad, $new_telefon, $userID);
        }
        if (mysqli_stmt_execute($stmt)) {
            $success = "Profil bilgileriniz başarıyla güncellendi.";
            $ad_soyad = $new_ad_soyad;
            $telefon = $new_telefon;
        } else {
...
/* Sidebar CSS (past-appts.php ile aynı) */
.sidebar {
    position: fixed;
    top: 0;
    left: 0;
...
<div class="container d-flex flex-column align-items-center justify-content-center" style="min-height: 90vh;">
    <div class="card profile-card rounded-4">
        <div class="profile-banner">
            ...
            <div class="desc">Bilgilerinizi güncelleyebilirsiniz.</div>
        </div>
        <div class="profile-form">
            ...
            <form method="post" autocomplete="off">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="telefon" name="telefon" value="<?= htmlspecialchars($telefon) ?>" required>
                </div>
                <div class="mb-3">
                    <label class="form-label" for="password">Yeni Şifre (isteğe bağlı)</label>
                    <input type="password" class="form-control" id="password" name="password" placeholder="Şifreyi değiştirmek için doldurun">
                </div>
                <button type="submit" class="btn btn-primary mt-2"><i class="bi bi-save"></i> Kaydet</button>
            </form>
        </div>
        <div class="profile-info">
            <i class="bi bi-info-circle me-2"></i>
            Ad Soyad ve Telefon zorunludur. Şifreyi değiştirmek istemiyorsanız boş bırakabilirsiniz.
        </div>
    </div>
</div>
<script>
